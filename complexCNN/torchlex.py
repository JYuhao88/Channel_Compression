...
def softplus(z):
    if "ComplexTensor" in z.__class__.__name__:
        a, b = z.real, z.imag
        real_ = torch.exp(a) * torch.cos(b) + 1
        imag_ = torch.exp(a) * torch.sin(b)
        r = torch.sqrt(real_**2 + imag_**2)
        theta = imag_
        z = ComplexTensor((torch.log(r), theta), complex=True, requires_grad=z.requires_grad)
    else:
        result = torch.nn.functional.softplus(z)
    return result
