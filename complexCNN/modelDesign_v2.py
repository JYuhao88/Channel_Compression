...
import torchsnooper
from torchlex import *
 
channelNum = 32

NUM_FEEDBACK_BITS = 440 
...
class Mish(torch.nn.Module):
    ...
    def forward(self, x):
        x = torchlex.ComplexTensor(x, requires_grad=True)
        x = x * (torchlex.tanh(torchlex.softplus(x)))
        return x
...
class ComplexConv(nn.Module):
    ...
    def forward(self, x): # shpae of x : [batch,2,channel,axis1,axis2]
        ...
        output = torch.complex(real,imaginary) # torch.view_as_complex(x)
        return output

def conv3x3(in_planes, out_planes, stride=1):
    """3x3 convolution with padding"""
    return ComplexConv(in_planes, out_planes, kernel_size=3, stride=stride,
                     padding=1, bias=True)
...
class ConvBN(nn.Sequential):
    def __init__(self, in_planes, out_planes, kernel_size, stride=1, groups=1):
        ...
        super(ConvBN, self).__init__(OrderedDict([
            ('conv', ComplexConv(in_planes, out_planes, kernel_size, stride,
...
class CRBlock64(nn.Module):
    def __init__(self):
        ...
        self.path2 = nn.Sequential(OrderedDict([
            ...
            ('conv1x1', ConvBN(channelNum * 2, channelNum * 2, 1)),
            ('conv3x3', ConvBN(channelNum * 2, channelNum * 2, 3)),
        ]))
...
class CRBlock(nn.Module):
    def __init__(self):
        ...
        self.path2 = nn.Sequential(OrderedDict([
            ...
            ("conv1x1_bn", ConvBN(channelNum, channelNum, 1)),
        ]))
...
class Encoder(nn.Module):
    ...
    def __init__(self, feedback_bits, quantization=True):
        ...
        self.convban = nn.Sequential(OrderedDict([
            ("conv3x3_bn", ConvBN(1, channelNum, 3)),
        ]))
    ...
    def forward(self, x):
        ...
        out = torch.cat((encode1, encode2), dim=2)
        out = self.encoder_conv(out)
        out = self.encoder_conv1(out)
...
class Decoder(nn.Module):
    ...
    def __init__(self, feedback_bits, quantization=True):
        ...
        self.out_cov = conv3x3(channelNum, 1)
        self.sig = nn.Sigmoid()
        self.quantization = quantization
...
def MSE_cuda(x, x_hat):
    ...
    x_hat_real = x_hat[:, :, :, 0].view(len(x_hat), -1) - 0.5
    x_hat_imag = x_hat[:, :, :, 1].view(len(x_hat), -1) - 0.5
    mse = torch.mean((x_real-x_hat_real)**2 + (x_imag-x_hat_imag)**2, axis=1)
    return mse
