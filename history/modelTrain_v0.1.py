...
import numpy as np
import torch
from modelDesign import AutoEncoder,DatasetFolder, NMSE #*
import os
...
for epoch in range(EPOCHS):
    ...
    for i, autoencoderInput in enumerate(train_loader):
        ...
        loss.backward()
        optimizer.step()
        # print(autoencoderOutput.cpu().detach().numpy().size())
        NMSE_ = NMSE(autoencoderInput.cpu().detach().numpy(), autoencoderOutput.cpu().detach().numpy())
        if i % PRINT_RREQ == 0:
            print('Epoch: [{0}][{1}/{2}]\t' 'Loss {loss:.7f}\t' 'NMSE {NMSE:.7f}\t'.format(epoch, i, len(train_loader), loss=loss.item(), NMSE = NMSE_))
    # Model Evaluating
