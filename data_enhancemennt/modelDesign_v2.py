...
def column_shuffle(x, shuffle_rate, shuffle_method='random', groups=4):
    batchsize, num_channels, height, width = x.data.size()
    num_shuffle = int(num_channels*shuffle_rate)
    x1 = x[:, :-num_shuffle, :, :]
    x2 = x[:, -num_shuffle:, :, :]
    if shuffle_method == 'random':
        x2 = x2[:, :, :, torch.randperm(16)]
    elif shuffle_method == 'grouping':
        width_per_group = width //groups
        x2 = x2.view(batchsize, num_shuffle, height, groups, width_per_group)
        x2 = torch.transpose(x2, 3, 4).contiguous()
        x2 = x2.view(batchsize, num_shuffle, height, -1)
    x = torch.cat((x1, x2), 1)
    return x
...
class Encoder_conv(nn.Module):
    ...
    def forward(self, input):
        ...
        x7 = column_shuffle(x7, 0.25, 'grouping')
