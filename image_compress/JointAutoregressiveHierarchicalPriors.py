...
class Encoder(nn.Module):
    ...
    def __init__(self, feedback_bits, N = 64, quantization=True):
        ...
        self.encoder = nn.Sequential(
            conv(2, N, kernel_size=5, stride=2),
            GDN(N),
            conv(N, N, kernel_size=5, stride=1),
            GDN(N),
            conv(N, N, kernel_size=5, stride=2),
            GDN(N),
            conv(N, 2, kernel_size=5, stride=1)
        )
...
class Decoder(nn.Module):
    ...
    def __init__(self, feedback_bits, N = 64, quantization=True):
        ...
        self.decoder_feature = nn.Sequential(
            deconv(2, N, kernel_size=5, stride=1),
            GDN(N, inverse=True),
            deconv(N, N, kernel_size=5, stride=2),
            GDN(N, inverse=True),
            deconv(N, N, kernel_size=5, stride=1),
            GDN(N, inverse=True),
            deconv(N, 2, kernel_size=5, stride=2)
        )
