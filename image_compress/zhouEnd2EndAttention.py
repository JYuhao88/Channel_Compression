...
class RB(nn.Module):
    ...
    def __init__(self, in_plane, conv_plane):
        ...
        self.conv = nn.Sequential(
            conv3x3(in_plane, conv_plane),
            nn.LeakyReLU(negative_slope=0.3, inplace=True),
            conv3x3(conv_plane, in_plane),
        )
    def forward(self, x):
        identity = x
        out = self.conv(x)
...
class Encoder(nn.Module):
    ...
    def __init__(self, feedback_bits, quantization=True):
        ...
        self.conv1 = conv3x3(N//2, N//8)
        self.layer2 = nn.Sequential(
            conv3x3(N//2, N//2),
            GDN(N//2))
        self.conv2 = conv3x3(N//2, N//4)
        self.layer3 = nn.Sequential(
            RNAB(N//2, N),
            conv3x3(N//2, N//2),
            GDN(N//2))
        self.conv3 = conv3x3(N//2, N//2)
        self.layer4 = nn.Sequential(
            RNAB(N//2, N),
            conv3x3(N//2, N//2),
            GDN(N//2))
        self.conv4 = conv1x1(N*11//8, N)
...
class Decoder(nn.Module):
    ...
    def forward(self, x):
        if self.quantization:
            ...
            out = x
        out = out.contiguous().view(-1, self.N)
        out = self.fc(out)
