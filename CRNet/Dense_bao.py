...
class Decoder(nn.Module):
    ...
    def __init__(self, feedback_bits, quantization=True):
        ...
        self.bottleneck_1 = nn.Sequential(OrderedDict([
            ("relu1_1", nn.LeakyReLU(negative_slope=0.3, inplace=False)),
            ("conv1x1_bn_1", ConvBN(64, 32, 1)),
            ("relu1_2", nn.LeakyReLU(negative_slope=0.3, inplace=False)),
        ]))
        self.bottleneck_2 = nn.Sequential(OrderedDict([
            ("relu2_1", nn.LeakyReLU(negative_slope=0.3, inplace=False)),
            ("conv1x1_bn_2", ConvBN(96, 32, 1)),
            ("relu2_2", nn.LeakyReLU(negative_slope=0.3, inplace=False)),
        ]))
        self.bottleneck_3 = nn.Sequential(OrderedDict([
            ("relu3_1", nn.LeakyReLU(negative_slope=0.3, inplace=False)),
            ("conv1x1_bn_3", ConvBN(128, 32, 1)),
            ("relu3_2", nn.LeakyReLU(negative_slope=0.3, inplace=False)),
        ]))
